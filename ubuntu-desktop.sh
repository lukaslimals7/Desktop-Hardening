...
function main {
  ...
  if grep -s "AUTOFILL='Y'" ./ubuntu-desktop.cfg; then
    ...
    sed -i "s/FW_ADMIN='/FW_ADMIN='$ADMINIP /" ./ubuntu-desktop.cfg
    sed -i "s/SSH_GRPS='/SSH_GRPS='$(id "$($WBIN -ih | awk '{print $1}' | head -n1)" -ng) /" ./ubuntu-desktop.cfg
    sed -i "s/CHANGEME=''/CHANGEME='$(date +%s)'/" ./ubuntu-desktop.cfg
    sed -i "s/VERBOSE='N'/VERBOSE='Y'/" ./ubuntu-desktop.cfg
  fi

  source ./ubuntu-desktop.cfg
...
LOGFILE="desktop-hardening-$(hostname --short)-$(date +%y%m%d).log"
echo "[DESKTOP HARDENING LOG - $(hostname --fqdn) - $(LANG=C date)]" >> "$LOGFILE"
